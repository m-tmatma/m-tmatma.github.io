...
int main(int argc, char* argv[])
{
	...
	*((int*)&CPUString[12]) = 0;
	...
	for (i=0; i<=nIds; ++i)
	{
		__cpuid(CPUInfo, i);
		printf_s("\nFor InfoType %d\n", i); 
		printf_s("CPUInfo[0] = 0x%08x\n", CPUInfo[0] );
		printf_s("CPUInfo[1] = 0x%08x\n", CPUInfo[1] );
		printf_s("CPUInfo[2] = 0x%08x\n", CPUInfo[2] );
		printf_s("CPUInfo[3] = 0x%08x\n", CPUInfo[3] );
		...
		if  (i == 1)
		{
			...
			bSSE3NewInstructions    = (CPUInfo[2] & 0x001) || false;
			bMONITOR_MWAIT          = (CPUInfo[2] & 0x008) || false;
			bCPLQualifiedDebugStore = (CPUInfo[2] & 0x010) || false;
